...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const { data } = await client.get<DeptSummary[]>('/kpi/dashboard-summary');
        setSummary(data);
      } catch {
  ...
  const canManage = !!employee && ['ADMIN', 'HR', 'MANAGER'].includes(employee.role);
    ...
    <div className="min-h-screen bg-white p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-5">
          {canManage && (
            <Link to="/kpi/sales" className="text-sm font-medium text-primary hover:text-deep">
              Log Sales KPIs
            </Link>
          )}
          {isAdmin && (
            <Link to="/team" className="text-sm font-medium text-primary hover:text-deep">
              Team
            </Link>
          )}
          ...
          <div className="bg-panel rounded-md p-5 h-80 border border-line">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={summary}>
                <XAxis dataKey="department" stroke="#5B6B63" tick={{ fontSize: 12 }} />
                <YAxis stroke="#5B6B63" tick={{ fontSize: 12 }} />
                <Tooltip contentStyle={{ background: '#FFFFFF', border: '1px solid #E3EAE6', borderRadius: 8 }} />
                <Bar dataKey="target" name="Target" fill="#DCE7DF" radius={[3, 3, 0, 0]} />
                <Bar dataKey="achieved" name="Achieved" fill="#087D43" radius={[3, 3, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </>
